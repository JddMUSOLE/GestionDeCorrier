...
Public Class Finstruction
    ...
    Sub fillGrid2()
        Try
            ...
            If (con.State = ConnectionState.Closed) Then
                con.Open()
                req = "SELECT * FROM Document WHERE CodeTypeDoc='0004'"
                cmd = New SqlCommand(req, con)
    ...
    Private Sub Finstruction_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        formLoadFields()
    ...
    Private Sub BtnEnregistrer_Click(sender As Object, e As EventArgs) Handles BtnEnregistrer.Click
        Try
            ...
            req = "INSERT INTO Document(NumOrdDoc,RefDoc,ConcerneDoc,DateDoc,DateReception,
                    Emplacement,CodeTypeDoc,OrganeExpBCC,Observation,
                    UserSecBCC) VALUES('" + TxtNum.Text.ToString.Trim + "','" + TxtRef.Text.ToString.Trim + "','" + TxtObjet.Text.ToString.Trim + "','" + TxtDateDoc.Value.Date + "',
                    '" + TxtDateRecept.Value.Date + "','" + CheminAcharger + "','" + typeDocumentIS + "','" + AutoriteSign + "','" + TxtObserv.Text.Trim + "','" + userMat + "')"
            If (con.State = ConnectionState.Closed) Then
                con.Open()
    ...
    Private Sub ComboAutoSign_DropDown(sender As Object, e As EventArgs) Handles ComboAutoSign.DropDown
        Try
            If (con.State = ConnectionState.Closed) Then
                con.Open()
