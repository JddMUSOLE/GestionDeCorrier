...
Public Class FormSelectOrientation
    ...
    Private Sub FormSelectOrientation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        formLoadFields()
    ...
    Private Sub TxtRef_TextChanged(sender As Object, e As EventArgs) Handles TxtRef.TextChanged
        Try
            ...
            If (con.State = ConnectionState.Closed) Then
                con.Open()
                req = "SELECT * FROM DataGridSelect WHERE RefDoc Like @Refnote"
                cmd = New SqlCommand(req, con)
                cmd.Parameters.AddWithValue("@Refnote", "%" & TxtRef.Text.Trim & "%")
                Using Dread1 As SqlDataReader = cmd.ExecuteReader
                    While Dread1.Read()
                        TxtNum = Dread1("NumOrdDoc").ToString()
                        txtObjet = Dread1("ConcerneDoc").ToString()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Dim courrierSrt As New CourrierSortant()
        courrierSrt.Show()
        Me.Hide()
    End Sub
